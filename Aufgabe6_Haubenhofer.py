...
def sucheWord(wordInput):                          #Funktion für das Suchen eines Wortes ob es nicht schon im Wörterbuch vorhanden ist.
    index = 0
    for wort in woerterbuch_deutsch:               #Wenn Wort in Woerterbuch_deutsch = wordinput, dann bricht die Schleife ab und gibt den Index aus.
    ...
    if index == len(woerterbuch_deutsch):
        index=0
        for wort in woerterbuch_englisch:          #Wenn Wort in Woerterbuch_englisch = wordinput, dann bricht die Schleife ab und gibt den Index aus.
            if wordInput.lower() == wort.lower():
                break
            index +=1    
        
        if index == len(woerterbuch_englisch):
            raise Exception("Das Wort steht nicht im Wörterbuch")   #Wenn nicht wird "Das Wort steht nicht im Wörterbuch" aus. 
...
def einfügenWord(deutschesWort, englischesWort):   #Funktion für das einfügen eines Wortes an einer beliebigen Stelle.
    try:
        ...
        Stelle = input("An der wie vielten Stelle soll das Wort im Wörterbuch eingefügt werden?: ")   #Wenn das Wort nicht vorhanden ist, wird das Wort an einer Stelle eingefügt.
...
def uebersetzenWord():
    try:
        output = sucheWord(input("Welches Wort wollen Sie übersetzen : "))              #Funktion für das Suchen eines Wortes ob es nicht schon im Wörterbuch vorhanden ist.
